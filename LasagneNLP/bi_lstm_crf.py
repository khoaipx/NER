...
from lasagne_nlp.networks.networks import build_BiLSTM_CRF
...
def main():
    ...
    logger = utils.get_logger("BiLSTM-CRF")
    ...
    bi_lstm_crf = build_BiLSTM_CRF(layer_incoming, num_units, num_labels, mask=layer_mask, grad_clipping=grad_clipping,
                                   peepholes=peepholes, dropout=dropout)

    logger.info("Network structure: hidden=%d" % (num_units))

    # compute loss
    num_tokens = mask_var.sum(dtype=theano.config.floatX)

    # get outpout of bi-lstm-cnn-crf shape [batch, length, num_labels, num_labels]
    energies_train = lasagne.layers.get_output(bi_lstm_crf)
    energies_eval = lasagne.layers.get_output(bi_lstm_crf, deterministic=True)

    loss_train = crf_loss(energies_train, target_var, mask_var).mean()
    loss_eval = crf_loss(energies_eval, target_var, mask_var).mean()
    # l2 regularization?
    if regular == 'l2':
        l2_penalty = lasagne.regularization.regularize_network_params(bi_lstm_crf, lasagne.regularization.l2)
    ...
    momentum = 0.9
    params = lasagne.layers.get_all_params(bi_lstm_crf, trainable=True)
    updates = utils.create_updates(loss_train, params, update_algo, learning_rate, momentum=momentum)

    # Compile a function performing a training step on a mini-batch
    train_fn = theano.function([input_var, target_var, mask_var], [loss_train, corr_train, num_tokens],
                               updates=updates)
